...
export default function () {
  ...
  group('Truck Controller Tests', () => {    
    ...
    group('Create Truck', () => {
      ...
      const res = http.post(`${BASE_URL}/trucks`, JSON.stringify({
      ...
      if (res.status === 201 || res.status === 200) {
        ...
        // Update Truck
        group('Update Truck', () => {
          const updateRes = http.put(`${BASE_URL}/trucks/${randomMacID}`, JSON.stringify({
        ...
        group('Get Truck Position History', () => {
          const historyRes = http.get(`${BASE_URL}/trucks/${createdTruck ? createdTruck.id : 1}/positions/limited`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });
        ...
        group('Get Truck Fuel History', () => {
          const fuelHistoryRes = http.get(`${BASE_URL}/trucks/${createdTruck ? createdTruck.id : 1}/fuel/limited`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });
        ...
        group('Get Truck Idle Detections', () => {
          const idleRes = http.get(`${BASE_URL}/trucks/${createdTruck ? createdTruck.id : 1}/idle-detections`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            },
          });
    ...
    group('Get All Trucks', () => {
      ...
      check(res, {
        ...
        'trucks data exists': (r) => JSON.parse(r.body).data !== undefined,
      });
    });
  });
