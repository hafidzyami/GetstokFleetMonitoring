...
const RouteHistoryPage = () => {
  ...
  const getStatusDisplay = (status: string) => {
    const statusMap: Record<string, { text: string, bgColor: string, textColor: string }> = {
      'planned': { text: 'Direncanakan', bgColor: 'bg-blue-100', textColor: 'text-blue-800' },
      'active': { text: 'Aktif', bgColor: 'bg-green-100', textColor: 'text-green-800' },
      'completed': { text: 'Selesai', bgColor: 'bg-gray-100', textColor: 'text-gray-800' },
      'cancelled': { text: 'Dibatalkan', bgColor: 'bg-red-100', textColor: 'text-red-800' }
    };
    ...
    <div className="h-full px-8 py-6 overflow-y-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800">Detail Rute #{routePlan.id}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <h2 className="font-semibold text-lg text-gray-800 mb-4">Informasi Rute</h2>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-lg text-gray-800">Titik Rute</h2>
                ...
                <div key={waypoint.id} className="flex items-start">
                  <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center mr-3
                    ${index === 0 ? 'bg-green-500' : 
                      index === routePlan.waypoints.length - 1 ? 'bg-red-500' : 'bg-blue-500'} 
                    text-white`}
                  >
                    {index}
                  </div>
          ...
          {(routePlan.avoidance_areas && routePlan.avoidance_areas.length > 0) && (
            <div className="bg-white rounded-lg shadow-sm p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="font-semibold text-lg text-gray-800">Area Dihindari</h2>
                ...
                {routePlan.avoidance_areas.map((area, index) => (
                  <div key={area.id} className="border border-gray-200 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="font-medium text-gray-800">Area {index + 1}</h3>
                      {area.is_permanent && (
                    ...
                    <p className="text-xs text-gray-500">{area.points.length} titik koordinat</p>
                  </div>
                ))}
              </div>
            </div>
