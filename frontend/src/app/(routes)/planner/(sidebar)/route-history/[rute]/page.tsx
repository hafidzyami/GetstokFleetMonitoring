...
const RouteHistoryPage = () => {
		...
		const fetchRoutePlanDetail = async () => {
			...
			try {
				...
				const response = await fetch(
					`/api/v1/route-plans/${params.rute}`,
					{
						headers: {
				...
				if (
					data.data.avoidance_areas &&
					data.data.avoidance_areas.length > 0
				) {
					const avoidanceMarkersGroups = data.data.avoidance_areas.map(
						(area) =>
							area.points.map((point) => ({
								id: `avoidance-${area.id}-${point.id}`,
								position: [point.latitude, point.longitude] as [
									number,
									number
								],
								reason: area.reason,
				...
				if (data.data.route_geometry) {
					...
					if (data.data.extras) {
						...
						if (
							data.data.extras.waytype &&
							data.data.extras.waytype.values
						) {
						...
						// Process tollways
						if (
							data.data.extras.tollways &&
							data.data.extras.tollways.values
						) {
						...
						if (
							data.data.extras.surface &&
							data.data.extras.surface.values
						) {
							...
							const surfaceTypesArray = Array(latLngs.length).fill(
								"Unknown"
							);
							for (const surface of surfaces) {
								...
								const surfaceType =
									surfaceMapping[surfaceValue] || "Unknown";
	...
	const handleApproveAvoidanceArea = async (areaId: number) => {
		...
		try {
			...
			if (approvedAreas.length > 0) {
				...
				try {
					...
					if (permanentResponse.ok) {
						...
						if (
							permanentResult.data &&
							Array.isArray(permanentResult.data)
						) {
							...
							const additionalPermanentAreas =
								permanentResult.data.filter(
					...
					console.error(
						"Error fetching permanent avoidance areas:",
						error
					);
			...
			if (routePlan && routePlan.avoidance_areas) {
				const updatedAvoidanceAreas = routePlan.avoidance_areas.map(
					(area) => {
						if (area.id === areaId) {
	...
	const handleRejectAvoidanceArea = async (areaId: number) => {
		...
		try {
			...
			const response = await fetch(
				`/api/v1/route-plans/avoidance/${areaId}`,
				{
					method: "DELETE",
					headers: {
						Authorization: `Bearer ${token}`,
	...
	const handleDeleteRoutePlan = async () => {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Error deleting route plan: ${response.statusText}`
				);
			}
	...
	if (!routePlan) {
		return (
			<div className="h-full flex flex-col items-center justify-center text-gray-500">
				...
				<p className="mb-4">
					Route plan dengan ID tersebut tidak ditemukan
				</p>
				<button
					onClick={() => router.push("/planner/riwayat-rute")}
					className="px-4 py-2 bg-blue-500 text-white rounded-md"
	...
	return (
		<div className="h-full sm:px-6 lg:px-8 py-4 sm:py-6 overflow-y-auto">
			{/* Header with back button and route info */}
			{/* mobil */}
			<div className="flex sm:hidden flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
				<div className="flex justify-between sm:justify-normal items-center gap-3">
					...
					<div className="flex w-[60%] gap-4">
						...
						<div className="flex flex-col gap-3">
							...
							<div>{getStatusDisplay(routePlan.status)}</div>
						</div>
					</div>
					{/* end left */}
					...
					<div>
						<button
							...
							className="flex items-center gap-2 sm:hidden px-3 sm:px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md transition-colors self-start sm:self-center"
						>
							<i className="bx bx-trash"></i>
							<span className="text-sm sm:text-base">Hapus Rute</span>
						</button>
					</div>
					{/* end right */}
				</div>
				...
				<div>
					<button
						onClick={() => handleDeleteRoutePlan()}
						className="hidden sm:flex items-center gap-2 px-3 sm:px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md transition-colors self-start sm:self-center"
					>
						<i className="bx bx-trash"></i>
						<span className="text-sm sm:text-base">Hapus Rute</span>
					</button>
      ...
      {routePlan.status === "on confirmation" && (
				<div className="px-2 justify-center items-center text-center mb-3 sm:hidden py-1 bg-yellow-100 border border-yellow-300 text-yellow-800 rounded-md text-xs sm:text-sm animate-pulse whitespace-nowrap">
      ...
      {/* end mobile */}
			...
			{/* desktop */}
			<div className="sm:flex hidden sm:flex-row sm:items-center justify-between gap-4 mb-6">
				<div className="flex justify-between sm:justify-normal items-center gap-3">
					{/* left */}
					<button
						onClick={() => router.push("/planner/route-history")}
					...
					<div>
						...
						<div className="flex flex-wrap items-center gap-1 sm:gap-2 text-sm text-gray-500 mt-1">
							<span>{formatDate(routePlan.created_at)}</span>
							<span className="hidden sm:inline">•</span>
			...
			{routePlan.status === "on confirmation" && (
				<div className="px-2 justify-center items-center text-center mb-3 hidden sm:flex py-1 bg-yellow-100 border border-yellow-300 text-yellow-800 rounded-md text-xs sm:text-sm animate-pulse whitespace-nowrap">
					<p className="text-center">
						Silakan konfirmasi area yang tidak bisa dilalui
					</p>
				</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
				...
				<div className="space-y-4 sm:space-y-6">
					...
					<div className="bg-white rounded-lg shadow-sm p-4 sm:p-6">
						<h2 className="font-semibold text-lg text-gray-800 mb-3 sm:mb-4">
							Informasi Rute
						</h2>

						<div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
							<div className="col-span-1 sm:col-span-2">
								<h3 className="text-gray-500 text-xs sm:text-sm mb-1">
									Dibuat Oleh
								</h3>
										...
										<div
											...
											onClick={() => {
												if (
													...
													const centerLat =
														latSum / area.points.length;
													const centerLng =
														lngSum / area.points.length;
													...
													const areaMarkers =
														impassibleMarkers.find(
															(markerGroup) => {
																if (markerGroup.length > 0) {
																	const idParts =
																		markerGroup[0].id.split(
																			"-"
																		);
																	return (
																		idParts.length >= 2 &&
										...
										>
											<div className="flex items-start justify-between gap-2 mb-2">
												...
												<div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
													{area.is_permanent && (
														<span className="px-1.5 py-0.5 bg-red-100 text-red-700 text-xs rounded whitespace-nowrap">
															Permanen
														</span>
													)}
													{area.status &&
														requesterRoles[
															area.requester_id as any
														] !== "planner" && (
															<span
													...
													>
														<i className="bx bx-trash text-sm sm:text-base"></i>
													</button>
												</div>
											</div>
			...
			{isProcessingAvoidanceArea && (
				<div className="fixed inset-0 bg-gray-50/90 z-[9999] flex items-center justify-center">
					<div className="bg-white p-6 sm:p-8 rounded-lg shadow-xl flex flex-col items-center max-w-xs sm:max-w-sm">
						<div className="w-16 h-16 sm:w-20 sm:h-20 border-4 border-t-blue-500 border-blue-200 rounded-full animate-spin mb-4 sm:mb-6"></div>
			...
			{isDeletingRoutePlan && (
				<div className="fixed inset-0 bg-gray-50/90 z-[9999] flex items-center justify-center">
					<div className="bg-white p-6 sm:p-8 rounded-lg shadow-xl flex flex-col items-center max-w-xs sm:max-w-sm">
						<div className="w-16 h-16 sm:w-20 sm:h-20 border-4 border-t-red-500 border-red-200 rounded-full animate-spin mb-4 sm:mb-6"></div>
						<p className="text-lg sm:text-xl font-semibold text-gray-800 text-center">
							Menghapus Rute...
						</p>
						<p className="text-xs sm:text-sm text-gray-500 mt-2 text-center">
